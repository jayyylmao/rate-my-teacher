...
import { CREATE_INTERVIEW_WITH_REVIEW_MUTATION } from "@/lib/graphql/mutations";
import { INTERVIEWS_QUERY, TAGS_QUERY } from "@/lib/graphql/queries";
import type {
  TagKey,
  TagDTO,
  RoundType,
...
export default function UnifiedReviewForm() {
  ...
  // Existing interview detection
  const [existingInterviews, setExistingInterviews] = useState<InterviewExperienceDTO[]>([]);
  ...
  const [hoveredStructuredRatings, setHoveredStructuredRatings] = useState<
    Record<keyof StructuredRatings, number>
  >({
    interviewStructure: 0,
    questionRelevance: 0,
    professionalism: 0,
  ...
  useEffect(() => {
    const loadTags = async () => {
      try {
        const data = await graphqlRequest<{ tags: { items: TagDTO[] } }>(TAGS_QUERY);
        setAvailableTags(data.tags.items);
      } catch (err) {
  ...
  // Search for existing interviews as user types
  useEffect(() => {
    if (searchDebounce) {
    ...
    const timeout = setTimeout(async () => {
      try {
        const query = `${company} ${role}`.trim();
        const data = await graphqlRequest<{
          interviews: { items: InterviewExperienceDTO[] };
        }>(INTERVIEWS_QUERY, { q: query, limit: 5 });

        const matches = data.interviews.items.filter((interview) => {
          const companyMatch =
            company.trim() &&
            interview.company.toLowerCase().includes(company.toLowerCase());
          const roleMatch =
            role.trim() && interview.role.toLowerCase().includes(role.toLowerCase());
          return companyMatch || roleMatch;
        });

        setExistingInterviews(matches);
        setShowingMatches(matches.length > 0);
      } catch (err) {
        console.error("Failed to search interviews:", err);
      }
    }, 500);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await graphqlRequest<{
        createInterviewWithReview: {
          interviewId: number;
          reviewId: number;
          status: string;
          isNewInterview: boolean;
        };
      }>(CREATE_INTERVIEW_WITH_REVIEW_MUTATION, {
        input: {
          // Interview fields
      ...
      // Redirect to interview page with success indicator
      router.push(`/interviews/${response.createInterviewWithReview.interviewId}?submitted=true`);
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {isAuthenticated ? (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <p className="text-green-800">
            Signed in as <span className="font-medium">{user?.email}</span>. This review will be
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-900 border-b pb-2">Interview Information</h2>
        ...
        {/* Existing Interviews Match Detection */}
        {showingMatches && existingInterviews.length > 0 && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-amber-900 mb-2">
                  {existingInterviews.length} existing interview
                  {existingInterviews.length !== 1 ? "s" : ""} found:
                </p>
                <ul className="text-sm text-amber-800 space-y-1">
                  {existingInterviews.slice(0, 3).map((interview) => (
                    <li key={interview.id}>
                      • {interview.company} - {interview.role} ({interview.reviewCount} review
                      {interview.reviewCount !== 1 ? "s" : ""})
                    </li>
                  ))}
                </ul>
                <p className="text-sm text-amber-700 mt-2">
                  Your review will be added to an existing interview if it matches exactly.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-6 pt-6 border-t">
        <h2 className="text-2xl font-bold text-gray-900 border-b pb-2">Your Review</h2>
                    ...
                    >
                      <svg
                        className={`w-8 h-8 transition-colors ${
                          value <=
                          (hoveredStructuredRatings[config.key] || structuredRatings[config.key])
                            ? "text-yellow-400"
                            : "text-gray-300"
        ...
        <div>
          ...
          <p
            className={`mt-1 text-sm ${
              comment.length < MIN_COMMENT_LENGTH ? "text-gray-500" : "text-green-600"
            }`}
          >
        ...
        <div>
          ...
          <input
            ...
            maxLength={100}
          />
          <p className="mt-1 text-sm text-gray-500">Leave blank to post anonymously</p>
        </div>

        {/* Interviewer Initials */}
        <div>
          <label
            htmlFor="interviewerInitials"
